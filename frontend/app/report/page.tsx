...
const CAREER_ICONS: Record<string, string> = {
  ...
  counselor: "💬", therapist: "🫶", biologist: "🌿", pilot: "✈️",
  army: "🎖️", defence: "🎖️", banker: "🏦", bank: "🏦",
};
...
function getClarityColor(clarity: string) {
  if (clarity === "Strongly Defined")   return { bg:"#dcfce7", color:"#16a34a" };
  if (clarity === "Moderately Defined") return { bg:"#dbeafe", color:"#2563eb" };
...
export default function ReportPage() {
  ...
  // Normalize top careers — supports both old and new API format
  const topCareers = (() => {
    ...
    <div style={{ minHeight:"100vh", background:"#f1f5f9", fontFamily:"'Segoe UI',sans-serif" }}>
      ...
      <div style={{ background:"white", borderBottom:"1px solid #e2e8f0", padding:"14px 32px", display:"flex", justifyContent:"space-between", alignItems:"center", boxShadow:"0 2px 8px rgba(0,0,0,0.06)", position:"sticky", top:0, zIndex:10 }}>
        ...
        <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
          <button onClick={handlePDF} disabled={pdfLoad}
            ...
            {pdfLoad ? (<><div style={{ width:14, height:14, border:"2px solid white", borderTopColor:"transparent", borderRadius:"50%", animation:"spin 0.8s linear infinite" }}/>Generating...</>) : "⬇ Download PDF"}
          </button>
          <button onClick={() => router.push("/dashboard")} style={{ padding:"10px 18px", background:"#f1f5f9", color:"#374151", border:"2px solid #e2e8f0", borderRadius:10, fontWeight:600, cursor:"pointer", fontSize:14, fontFamily:"inherit" }}>← Dashboard</button>
          <button onClick={handleLogout} style={{ padding:"10px 18px", background:"white", color:"#ef4444", border:"2px solid #fecaca", borderRadius:10, fontWeight:600, cursor:"pointer", fontSize:14, fontFamily:"inherit" }}>Logout</button>
        </div>
      </div>
      ...
      <div style={{ maxWidth:1100, margin:"0 auto", padding:"32px 20px" }}>
        ...
        <div style={{ background:"white", borderRadius:20, padding:"24px 32px", marginBottom:24, boxShadow:"0 2px 16px rgba(0,0,0,0.06)", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:16 }}>
          ...
          <div style={{ display:"flex", gap:24, flexWrap:"wrap", alignItems:"center" }}>
            {result.user.school && <div><p style={{ margin:0, fontSize:11, color:"#94a3b8", fontWeight:600, textTransform:"uppercase", letterSpacing:1 }}>School</p><p style={{ margin:"2px 0 0", fontWeight:600, color:"#374151", fontSize:14 }}>{result.user.school}</p></div>}
            {result.user.student_class && <div><p style={{ margin:0, fontSize:11, color:"#94a3b8", fontWeight:600, textTransform:"uppercase", letterSpacing:1 }}>Class</p><p style={{ margin:"2px 0 0", fontWeight:600, color:"#374151", fontSize:14 }}>{result.user.student_class}</p></div>}
            {completedDate && <div><p style={{ margin:0, fontSize:11, color:"#94a3b8", fontWeight:600, textTransform:"uppercase", letterSpacing:1 }}>Completed</p><p style={{ margin:"2px 0 0", fontWeight:600, color:"#374151", fontSize:14 }}>{completedDate}</p></div>}
            <div>
              <p style={{ margin:0, fontSize:11, color:"#94a3b8", fontWeight:600, textTransform:"uppercase", letterSpacing:1 }}>Personality</p>
              <span style={{ background:clarityColor.bg, color:clarityColor.color, padding:"3px 10px", borderRadius:20, fontSize:12, fontWeight:700 }}>{clarity}</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ background:"linear-gradient(135deg,#667eea 0%,#764ba2 100%)", borderRadius:20, padding:"40px 48px", marginBottom:24, display:"flex", justifyContent:"space-between", alignItems:"center", boxShadow:"0 8px 32px rgba(102,126,234,0.35)", flexWrap:"wrap", gap:20 }}>
          <div>
            ...
            <h1 style={{ fontSize:32, fontWeight:800, color:"white", margin:"0 0 10px" }}>{topCareers[0]?.career || result.top_career}</h1>
        ...
        {topCareers.length > 0 && (
          <div style={{ background:"white", borderRadius:20, padding:"28px 32px", marginBottom:24, boxShadow:"0 2px 16px rgba(0,0,0,0.06)" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:24, flexWrap:"wrap", gap:8 }}>
              <div>
                <h3 style={{ fontWeight:800, color:"#1e293b", fontSize:18, margin:"0 0 4px" }}>🏆 Top {topCareers.length} Career Recommendations</h3>
                <p style={{ color:"#94a3b8", fontSize:13, margin:0 }}>Ranked by multi-trait weighted confidence based on your full psychometric profile</p>
              </div>
              <div style={{ background:"linear-gradient(135deg,#667eea15,#764ba215)", border:"1.5px solid #667eea30", borderRadius:10, padding:"6px 14px" }}>
                <span style={{ fontSize:12, fontWeight:700, color:"#667eea" }}>WEIGHTED RANKING</span>
              </div>
            </div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              {topCareers.map((entry, idx) => {
                ...
                const isTop   = idx === 0;
                return (
                  <div key={idx} style={{ display:"flex", alignItems:"center", gap:18, padding:isTop?"22px 24px":"16px 20px", borderRadius:16, background:isTop?`linear-gradient(135deg,${grad.from}12,${grad.to}08)`:"#f8fafc", border:isTop?`2px solid ${grad.from}40`:"2px solid #f1f5f9", position:"relative", overflow:"hidden" }}>
                    <div style={{ position:"absolute", right:20, top:"50%", transform:"translateY(-50%)", fontSize:isTop?72:52, fontWeight:900, color:grad.from+"0d", lineHeight:1, userSelect:"none", pointerEvents:"none" }}>#{idx+1}</div>
                    <div style={{ width:isTop?52:42, height:isTop?52:42, borderRadius:14, background:`linear-gradient(135deg,${grad.from},${grad.to})`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:isTop?22:18, flexShrink:0, boxShadow:`0 4px 12px ${grad.from}50` }}>{icon}</div>
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:isTop?8:6 }}>
                        <span style={{ fontWeight:isTop?800:700, fontSize:isTop?17:15, color:"#1e293b", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{entry.career}</span>
                        <span style={{ background:`linear-gradient(135deg,${grad.from},${grad.to})`, color:"white", padding:"2px 10px", borderRadius:20, fontSize:11, fontWeight:700, flexShrink:0 }}>{RANK_LABELS[idx]}</span>
                      </div>
                      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:6, flexShrink:0 }}>
                          <span style={{ fontWeight:800, color:"#1e293b", fontSize:isTop?16:14 }}>{entry.confidence.toFixed(0)}%</span>
                          <span style={{ background:confLbl.bg, color:confLbl.color, padding:"2px 8px", borderRadius:20, fontSize:11, fontWeight:700 }}>{confLbl.label}</span>
                        </div>
                      </div>
                      {isTop && <p style={{ margin:"6px 0 0", fontSize:12, color:"#64748b" }}>✨ Strongest match based on your intelligence profile, RIASEC personality and interest patterns</p>}
                    </div>
                  </div>
                );
              })}
            ...
            <div style={{ marginTop:18, padding:"12px 16px", background:"#f8fafc", borderRadius:10, border:"1px solid #e2e8f0" }}>
              <p style={{ margin:0, fontSize:12, color:"#64748b", lineHeight:1.6 }}>
                <strong style={{ color:"#374151" }}>ℹ️ How confidence is calculated:</strong> The percentage reflects how strongly your combined Multiple Intelligence scores and RIASEC personality traits align with each career's typical professional profile using multi-trait weighted scoring.
              </p>
            </div>
          </div>
        )}
            ...
            {intelData.map((item, i) => {
              ...
              return (
                <div key={item.subject} style={{ display:"flex", alignItems:"center", gap:14, padding:"16px 18px", background:i===0?"#f8f7ff":"#f8fafc", borderRadius:12, border:i===0?"2px solid #667eea30":"2px solid transparent" }}>
                  <div style={{ width:38, height:38, borderRadius:10, background:color+"20", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, fontSize:18 }}>{icon}</div>
                  <div style={{ flex:1, minWidth:0 }}>
        ...
        <div style={{ display:"flex", gap:12, justifyContent:"center", paddingBottom:32, flexWrap:"wrap" }}>
          ...
          <button onClick={() => router.push("/dashboard")} style={{ padding:"14px 32px", borderRadius:12, background:"white", color:"#374151", border:"2px solid #e2e8f0", fontWeight:700, fontSize:15, cursor:"pointer", fontFamily:"inherit" }}>← Back to Dashboard</button>
          <button onClick={handleLogout} style={{ padding:"14px 32px", borderRadius:12, background:"linear-gradient(135deg,#ef4444,#dc2626)", color:"white", border:"none", fontWeight:700, fontSize:15, cursor:"pointer", fontFamily:"inherit" }}>Logout</button>
        </div>
      </div>
    </div>
  );
